...
int RunBET(float *BrainData,int NX, int NY, int NZ, float *Mask){


	double xarg = 0, yarg = 0, zarg = 0;
	
    volume<float> testvol(NX,NY,NZ);
    
	for (int  l = 0; l < NX; l++ ){
		for (int  m = 0; m < NY; m++ ){
			for (int  n = 0; n < NZ; n++ ){
				testvol(l,m,n) = BrainData[l+NX*m+NX*NY*n];
			}
		}
	}
	
	
    double bet_main_parameter = pow(fractional_threshold.value(), .275);
    
	string out = "Test_brain";
  ...
  if (generate_mask.value().size()>0){
	...
	for (int  l = 0; l < NX; l++ ){
		for (int  m = 0; m < NY; m++ ){
			for (int  n = 0; n < NZ; n++ ){
				Mask[l+NX*m+NX*NY*n] = 1.0-brainmask(l,m,n);
			}
		}
	}
...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
	...
	/////////////////////////////////////////////////////////////////
    if (!(nrhs==1 || nrhs==2) ){
        mexErrMsgTxt("One or two input arguments!\n");
    }
	...
	if (nrhs==2){
		...
		// Outline
		if ((mx = mxGetField(prhs[1], 0, "outline"))) {
		...
		if ((mx = mxGetField(prhs[1], 0, "fractional_threshold"))) {
			double *temp = mxGetPr(mx);
			const string tempString = to_string(temp[0]);
			fractional_threshold.set_value(tempString);
		}
		
		// Skull
		if ((mx = mxGetField(prhs[1], 0, "skull"))) {
			double *temp = mxGetPr(mx);
			if(temp[0])
				skull.set_T(1);
			else
		...
		// smootharg
		if ((mx = mxGetField(prhs[1], 0, "smooth"))) {
			double *temp = mxGetPr(mx);
			const string tempString = to_string(temp[0]);
			smootharg.set_value(tempString);
		}
	...
	// Input data
	/////////////////////////////////////////////////////////////////
    float *BrainData = (float*)mxGetPr(prhs[0]);
    
    const mwSize *dims = mxGetDimensions(prhs[0]);
    mwSize number_of_dimensions = mxGetNumberOfDimensions(prhs[0]);
	
	if (mxIsComplex(prhs[0]))
		mexErrMsgTxt("Only magnitude data!\n"); 
		
    if (number_of_dimensions!=3)
        mexErrMsgTxt("3D data expected!\n"); 
    ...
    float *Mask   = (float*)mxGetPr(plhs[0]);
       	
    
	RunBET(BrainData,NX, NY, NZ, Mask);
    
	
	///usr/local/fsl/bin/bet /home/fsl/Desktop/ImgTest /home/fsl/Desktop/ImgTest_brain -A -f 0.5 -g 0 -m
	delete[] Dims;
}
