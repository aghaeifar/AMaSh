...
%
    ...
    myAlgList = {'interior-point', 'sqp', 'interior-point'};
    ...
    elseif myAlgorithm == 3
        options = optimoptions('fmincon','SpecifyObjectiveGradient',false, 'Display', showProgress, 'Algorithm', myAlgList{myAlgorithm}, 'MaxFunEvals', 10^6, 'MaxIter', 100000);  
        coef = fmincon(@(x) minimizeSTDFunCore(x, MapB0, MAP_Shims), zeros(1, size(MAP_Shims, 1)), [], [], [], [], currLimL, currLimU, [], options);
    end
...
function err = minimizeSTDFunCore(x, MapB0, Map_Shims)
    err = std(MapB0 + x*Map_Shims);
end