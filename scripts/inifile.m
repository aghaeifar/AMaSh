...
if strcmpi(line(1),';')                     % comment found
    status = 4;
    value = line(2:end);
elseif strcmpi(line(1),'#')                     % comment found
    status = 4;
    value = line(2:end);
elseif (line(1) == '[')  & (length(line) >= 3)  % section found
    % Check if there is a trailing comment
    CPos = strfind(line,']');
    if(isempty(CPos))
        value = lower(line(2:end-1));
    else
        value = lower(line(2:CPos(1)-1));
    end
    status = 1;
